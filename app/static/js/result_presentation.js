(function () {
  ...
  document.getElementById("startRevealButton").addEventListener("click", async () => {
    ...
    if (summary) await runRevealSequence(summary);
  });
  ...
  async function init() {
    ...
    if (summary.project.status === "LOCKED") {
      standbyPanel.classList.remove("hidden");
    } else if (summary.project.status === "PRESENTING") {
      // 再訪問時は毎回演出を最初からやり直す(演出の途中状態は永続化しない)
      await runRevealSequence(summary);
    } else if (summary.project.status === "FINISHED") {
      showRanking(summary);
    }
  }
